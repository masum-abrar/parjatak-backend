...
export const createList = async (req, res) => {
  try {
    return await prisma.$transaction(async (tx) => {
      ...
      //validate input
      const inputValidation = validateInput([userId, title], ["User", "Title"]);
      ...
      const product = await tx.list.findFirst({
        where: {
...
export const createListPlace = async (req, res) => {
  try {
    return await prisma.$transaction(async (tx) => {
      const { userId, listId, placeId, isActive } = req.body;
      ...
      // for (let i = 0; i < requestBodyLength; i++) {
      //check if product exists
      const product = await tx.listPlace.findFirst({
        where: {
          //   userId: req.user.parentId ? req.user.parentId : req.user.id,
          placeId: placeId,
          isActive: true,
      ...
      if (product && product.placeId === placeId)
        return res
          .status(409)
          .json(jsonResponse(false, `This place already added.`, null));
      ...
      if (!req.files || req.files.length === 0) {
        //create products
        let newProduct = await prisma.listPlace.create({
          data: {
            userId: userId,
            placeId,
            listId,
        ...
        if (!newProduct) {
          return res
            ...
            .json(
              jsonResponse(
                false,
                `This place cannot be added to the list`,
                null
              )
            );
        }
        ...
        if (newProduct) {
          return res
            .status(200)
            .json(
              jsonResponse(true, "Place has been added to the list", newProduct)
            );
        }
      }
        ...
        async (error, result) => {
          ...
          if (req.files.length === newImages.length) {
            ...
            if (newProduct) {
              return res
                .status(200)
                .json(jsonResponse(true, "Place has been added", newProduct));
            }
          }
        }
      );
...
export const deleteList = async (req, res) => {
  try {
    return await prisma.$transaction(async (tx) => {
      const product = await tx.list.delete({
        where: { id: req.params.id },
        // data: { deletedBy: req.user.id, isDeleted: true },
      });

      if (product) {
        const productImage = await prisma.listImage.findMany({
          where: { listId: req.params.id },
        });
        ...
        return res
          .status(200)
          .json(jsonResponse(true, `List has been deleted`, product));
      } else {
        return res
          .status(404)
          .json(jsonResponse(false, "List has not been deleted", null));
      }
    });
  } catch (error) {
    console.log(error);
...
export const getListsForCustomer = async (req, res) => {
  try {
    ...
    if (products) {
      return res
        .status(200)
        .json(jsonResponse(true, `${products.length} lists found`, products));
    } else {
      return res
        .status(404)
        .json(jsonResponse(false, "Something went wrong. Try again", null));
...
export const getListForCustomer = async (req, res) => {
  try {
    const product = await prisma.list.findFirst({
      ...
      include: {
        place: {
          select: { id: true, name: true, images: { select: { image: true } } },
        },
...
export const deleteListLike = async (req, res) => {
  try {
    return await prisma.$transaction(async (tx) => {
      ...
      const revRes = await tx.listLike.findFirst({
...
export const getListReviews = async (req, res) => {
  try {
    const products = await prisma.listReview.findMany({
      where: {
        isActive: true,
        // AND: [
        //   {
    ...
    if (products.length === 0)
      return res
        .status(200)
        .json(jsonResponse(true, "No list review is available", null));

    if (products) {
      return res
        .status(200)
        .json(
          jsonResponse(true, `${products.length} list reviews found`, products)
        );
    } else {
      return res
        .status(404)
