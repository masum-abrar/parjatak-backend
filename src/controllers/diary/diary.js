...
export const createDiary = async (req, res) => {
  try {
    return await prisma.$transaction(async (tx) => {
      const { userId, placeId, description, title, isActive, targetDate } =
        req.body;

      //validate input
      const inputValidation = validateInput(
        [userId, placeId, title],
        ["User", "Place", "Title"]
      );
      ...
      const product = await tx.diary.findFirst({
        where: {
      ...
      if (!req.files || req.files.length === 0) {
        //create products
        let newProduct = await prisma.diary.create({
          data: {
            userId: userId,
            placeId,
            description,
        ...
        if (newProduct) {
          return res
            .status(200)
            .json(jsonResponse(true, "Diary has been created", newProduct));
        }
      }
...
export const getDiaries = async (req, res) => {
  if (req.user.roleName !== "super-admin") {
    ...
    try {
      const products = await prisma.diary.findMany({
        where: {
          //   isActive: true,
          AND: [
            {
      ...
      if (products.length === 0)
        return res
          .status(200)
          .json(jsonResponse(true, "No diary is available", null));

      if (products) {
        return res
          .status(200)
          .json(
            jsonResponse(true, `${products.length} diaries found`, products)
          );
      } else {
        return res
          .status(404)
...
export const getDiariesByUser = async (req, res) => {
  try {
    ...
    if (products) {
      return res
        .status(200)
        .json(jsonResponse(true, `${products.length} diaries found`, products));
    } else {
      return res
        .status(404)
        .json(jsonResponse(false, "Something went wrong. Try again", null));
...
export const getDiary = async (req, res) => {
  try {
    const product = await prisma.diary.findFirst({
    ...
    if (product) {
      return res.status(200).json(jsonResponse(true, `1 diary found`, product));
    } else {
      return res
        .status(404)
        .json(jsonResponse(false, "No diary is available", null));
    }
  } catch (error) {
    console.log(error);
    return res.status(500).json(jsonResponse(false, error, null));
...
export const updateDiary = async (req, res) => {
  try {
    return await prisma.$transaction(async (tx) => {
      ...
      const findProduct = await tx.diary.findFirst({
      ...
      if (!findProduct)
        return res
          .status(404)
          .json(jsonResponse(false, "This diary does not exist", null));
      ...
      if (title) {
        if (title !== findProduct.title) {
          const existingProduct = await tx.diary.findFirst({
            where: {
              //   userId: req.user.parentId ? req.user.parentId : req.user.id,
              title: title,
              isActive: true,
      ...
      //update product
      const product = await tx.diary.update({
        where: { id: req.params.id },
        data: {
          //   userId: req.user.parentId ? req.user.parentId : req.user.id,
          //   userId,
      ...
      if (product) {
        if (req.files) {
          ...
          //max 3 image
          const productImages = await tx.diaryImage.findMany({
          ...
          await uploadToCLoudinary(
            ...
            async (error, result) => {
              ...
              if (imagesLength === newImages.length) {
                ...
                return res
                  .status(200)
                  .json(jsonResponse(true, `Diary has been updated`, product));
              }
            }
          );
        } else {
        ...
        return res
          .status(404)
          .json(jsonResponse(false, "Diary has not been updated", null));
      }
    });
  } catch (error) {
    console.log(error);
...
export const updateDiaryImage = async (req, res) => {
  try {
    return await prisma.$transaction(async (tx) => {
      const { image } = req.body;

      const findProductImage = await tx.diaryImage.findFirst({
        where: { id: req.params.id },
      });
      //upload image
      ...
      await uploadToCLoudinary(req.file, module_name, async (error, result) => {
        ...
        //update product image
        const productImage = await prisma.diaryImage.update({
          where: { id: req.params.id },
          data: {
        ...
        if (productImage) {
          ...
          return res
            .status(200)
            .json(
              jsonResponse(true, `Diary image has been updated`, productImage)
            );
        } else {
          return res
            .status(404)
            .json(
              jsonResponse(false, "Diary image has not been updated", null)
            );
        }
      });
    });
...
export const deleteDiaryImage = async (req, res) => {
  try {
    return await prisma.$transaction(async (tx) => {
      const productImage = await tx.diaryImage.delete({
        where: { id: req.params.id },
      ...
      if (productImage) {
        ...
        return res
          .status(200)
          .json(
            jsonResponse(true, `Diary image has been deleted`, productImage)
          );
      } else {
        return res
          .status(404)
          .json(jsonResponse(false, "Diary image has not been deleted", null));
      }
    });
  } catch (error) {
    console.log(error);
...
export const deleteDiary = async (req, res) => {
  try {
    return await prisma.$transaction(async (tx) => {
      const product = await tx.diary.delete({
        where: { id: req.params.id },
        // data: { deletedBy: req.user.id, isDeleted: true },
      });

      if (product) {
        const productImage = await prisma.diaryImage.findMany({
          where: { diaryId: req.params.id },
        });
        ...
        return res
          .status(200)
          .json(jsonResponse(true, `Diary has been deleted`, product));
      } else {
        return res
          .status(404)
          .json(jsonResponse(false, "Diary has not been deleted", null));
      }
    });
  } catch (error) {
    console.log(error);
...
//For Customer
//get all products
export const getDiariesForCustomer = async (req, res) => {
  try {
    const products = await prisma.diary.findMany({
      where: {
        isActive: true,
...
export const getDiaryForCustomer = async (req, res) => {
  try {
    const product = await prisma.diary.findFirst({
      where: {
    ...
    if (product) {
      const productUpdate = await prisma.diary.update({
        where: {
          id: product?.id,
        },
        data: {
...
//get all images
export const getDiaryImages = async (req, res) => {
  // if (req.user.roleName !== "super-admin") {
  //   getProductsByUser(req, res);
  // } else {
  try {
    const products = await prisma.diaryImage.findMany({
      where: {
        diaryId: req.params.id,
        // isDeleted: false,
        // isActive: true,
...
//create an image
export const createDiaryImage = async (req, res) => {
  try {
    return await prisma.$transaction(async (tx) => {
      let { diaryId } = req.body;

      //   console.log(req.body);

      //validate input
      const inputValidation = validateInput([diaryId], ["Diary Id"]);
      ...
      await uploadToCLoudinary(req.file, module_name, async (error, result) => {
        ...
        //create brand
        const newProductImage = await prisma.diaryImage.create({
          data: {
            diaryId,
            image: result.secure_url,
          },
