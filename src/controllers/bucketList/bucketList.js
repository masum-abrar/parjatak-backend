...
const module_name = "bucketList";

//create product
export const createBucketList = async (req, res) => {
  try {
    return await prisma.$transaction(async (tx) => {
      const { userId, placeId, description, title, targetDate, isActive } =
        req.body;

      //validate input
      const inputValidation = validateInput(
        [userId, placeId, title, targetDate],
        ["User", "Place", "Title", "Target Date"]
      );
      ...
      const product = await tx.bucketList.findFirst({
        where: {
      ...
      if (!req.files || req.files.length === 0) {
        //create products
        let newProduct = await prisma.bucketList.create({
          data: {
            userId: userId,
            placeId,
            description,
        ...
        if (newProduct) {
          return res
            .status(200)
            .json(
              jsonResponse(true, "Bucket list has been created", newProduct)
            );
        }
      }
...
export const getBucketLists = async (req, res) => {
  if (req.user.roleName !== "super-admin") {
    ...
    try {
      const products = await prisma.bucketList.findMany({
        where: {
          //   isActive: true,
          AND: [
            {
      ...
      if (products.length === 0)
        return res
          .status(200)
          .json(jsonResponse(true, "No bucket list is available", null));

      if (products) {
        return res
          .status(200)
          .json(
            jsonResponse(true, `${products.length} bucket list found`, products)
          );
      } else {
        return res
          .status(404)
...
export const getBucketListsByUser = async (req, res) => {
  try {
    const products = await prisma.bucketList.findMany({
      where: {
        userId: req.user.parentId ? req.user.parentId : req.user.id,
...
export const getBucketList = async (req, res) => {
  try {
    const product = await prisma.bucketList.findFirst({
    ...
    if (product) {
      return res
        .status(200)
        .json(jsonResponse(true, `1 bucket list found`, product));
    } else {
      return res
        .status(404)
        .json(jsonResponse(false, "No bucket list is available", null));
    }
  } catch (error) {
    console.log(error);
    return res.status(500).json(jsonResponse(false, error, null));
...
export const updateBucketList = async (req, res) => {
  try {
    return await prisma.$transaction(async (tx) => {
      ...
      //validate input
      const inputValidation = validateInput(
        [title, userId, placeId, targetDate],
        ["Title", "User", "Place", "Target Date"]
      );
      ...
      const findProduct = await tx.bucketList.findFirst({
      ...
      if (!findProduct)
        return res
          .status(404)
          .json(jsonResponse(false, "This bucket list does not exist", null));
      ...
      if (title) {
        if (title !== findProduct.title) {
          const existingProduct = await tx.bucketList.findFirst({
            where: {
              //   userId: req.user.parentId ? req.user.parentId : req.user.id,
              title: title,
              isActive: true,
      ...
      //update product
      const product = await tx.bucketList.update({
        where: { id: req.params.id },
        data: {
          //   userId: req.user.parentId ? req.user.parentId : req.user.id,
          //   userId,
      ...
      if (product) {
        if (req.files) {
          ...
          //max 3 image
          const productImages = await tx.bucketListImage.findMany({
            where: { bucketListId: req.params.id },
          });
              ...
              if (imagesLength === newImages.length) {
                if (Array.isArray(imagesLength) && imagesLength > 0) {
                  for (let i = 0; i < imagesLength; i++) {
                    await prisma.bucketListImage.create({
                      data: {
                ...
                return res
                  .status(200)
                  .json(
                    jsonResponse(true, `Bucket List has been updated`, product)
                  );
              }
            }
          );
          ...
          return res
            .status(200)
            .json(jsonResponse(true, `Bucket list has been updated`, product));
        }
      } else {
        return res
          .status(404)
          .json(jsonResponse(false, "Bucket list has not been updated", null));
      }
    });
  } catch (error) {
    console.log(error);
...
export const updateBucketListImage = async (req, res) => {
  try {
    return await prisma.$transaction(async (tx) => {
      const { image } = req.body;

      const findProductImage = await tx.bucketListImage.findFirst({
        where: { id: req.params.id },
      });
      //upload image
      ...
      await uploadToCLoudinary(req.file, module_name, async (error, result) => {
        ...
        //update product image
        const productImage = await prisma.bucketListImage.update({
          where: { id: req.params.id },
          data: {
        ...
        if (productImage) {
          ...
          return res
            ...
            .json(
              jsonResponse(
                true,
                `Bucket list image has been updated`,
                productImage
              )
            );
        } else {
          return res
            ...
            .json(
              jsonResponse(
                false,
                "Bucket list image has not been updated",
                null
              )
            );
        }
...
export const deleteBucketListImage = async (req, res) => {
  try {
    return await prisma.$transaction(async (tx) => {
      const productImage = await tx.bucketListImage.delete({
        where: { id: req.params.id },
      ...
      if (productImage) {
        ...
        return res
          ...
          .json(
            jsonResponse(
              true,
              `Bucket list image has been deleted`,
              productImage
            )
          );
      } else {
        return res
          .status(404)
          .json(
            jsonResponse(false, "Bucket list image has not been deleted", null)
          );
      }
    });
  } catch (error) {
...
export const deleteBucketList = async (req, res) => {
  try {
    return await prisma.$transaction(async (tx) => {
      const product = await tx.bucketList.delete({
        where: { id: req.params.id },
        // data: { deletedBy: req.user.id, isDeleted: true },
      });

      if (product) {
        const productImage = await prisma.bucketListImage.findMany({
          where: { bucketListId: req.params.id },
        });
        ...
        return res
          .status(200)
          .json(jsonResponse(true, `Bucket list has been deleted`, product));
      } else {
        return res
          .status(404)
          .json(jsonResponse(false, "Bucket list has not been deleted", null));
      }
    });
  } catch (error) {
    console.log(error);
...
//For Customer
//get all products
export const getBucketListsForCustomer = async (req, res) => {
  try {
    const products = await prisma.bucketList.findMany({
      where: {
        isActive: true,
    ...
    if (products) {
      return res
        .status(200)
        .json(
          jsonResponse(true, `${products.length} bucket lists found`, products)
        );
    } else {
      return res
        .status(404)
...
export const getBucketListForCustomer = async (req, res) => {
  try {
    const product = await prisma.bucketList.findFirst({
      where: {
    ...
    if (product) {
      const productUpdate = await prisma.bucketList.update({
        where: {
          id: product?.id,
        },
        data: {
...
//get all images
export const getBucketListImages = async (req, res) => {
  // if (req.user.roleName !== "super-admin") {
  //   getProductsByUser(req, res);
  // } else {
  try {
    const products = await prisma.bucketListImage.findMany({
      where: {
        bucketListId: req.params.id,
        // isDeleted: false,
        // isActive: true,
...
//create an image
export const createBucketListImage = async (req, res) => {
  try {
    return await prisma.$transaction(async (tx) => {
      let { bucketListId } = req.body;

      //   console.log(req.body);

      //validate input
      const inputValidation = validateInput([bucketListId], ["Bucket list Id"]);
      ...
      await uploadToCLoudinary(req.file, module_name, async (error, result) => {
        ...
        //create brand
        const newProductImage = await prisma.bucketListImage.create({
          data: {
            bucketListId,
            image: result.secure_url,
          },
        ...
        if (newProductImage) {
          return res
            ...
            .json(
              jsonResponse(
                true,
                "Bucket list image has been uploaded",
                newProductImage
              )
            );
        }
...
export const createBucketListLike = async (req, res) => {
  try {
    return await prisma.$transaction(async (tx) => {
      ...
      //validate input
      const inputValidation = validateInput([bucketListId], ["Bucket List"]);
      ...
      const newReview = await prisma.bucketListLike.create({
        data: {
          bucketListId,
          userId: userId,
      ...
      if (newReview) {
        return res
          .status(200)
          .json(
            jsonResponse(true, "You have liked this bucket list", newReview)
          );
      }
    });
  } catch (error) {
...
export const deleteBucketListLike = async (req, res) => {
  try {
    return await prisma.$transaction(async (tx) => {
      let { bucketListId, userId, parentUserId } = req.body;

      const review = await tx.bucketListLike.delete({
      ...
      if (review) {
        ...
        return res
          ...
          .json(
            jsonResponse(
              true,
              `You have removed like from this bucket list`,
              review
            )
          );
      } else {
...
export const createPlaceReview = async (req, res) => {
  try {
    return await prisma.$transaction(async (tx) => {
      let { placeId, userId, rating, comment } = req.body;
      ...
      if (newReview) {
        return res
          .status(200)
          .json(jsonResponse(true, "You have reviewed this place", newReview));
      }
    });
  } catch (error) {
    console.log(error);
