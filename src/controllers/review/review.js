...
export const createReview = async (req, res) => {
  try {
    return await prisma.$transaction(async (tx) => {
      ...
      const existingReview = await tx.placeReview.findFirst({
        where: {
          placeId,
          userId,
        },
      ...
      if (existingReview) {
        return res
          .status(409)
          .json(jsonResponse(false, "You have already submitted a review.", null));
      }

      // ✅ Create the review
      const newReview = await tx.placeReview.create({
        data: {
          placeId,
          userId,
          rating, // already number
          comment,
        },
      });
      ...
      if (files.length > 7) {
        return res
          .status(400)
          .json(jsonResponse(false, "Maximum 7 images allowed", null));
      }
      ...
      for (const file of files) {
        try {
          ...
          await tx.placeReviewImage.create({
            data: {
      ...
      return res.status(200).json(
        jsonResponse(true, "Thank you for your review", {
          ...newReview,
